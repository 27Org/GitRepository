Ext.define('app.expand.ux.BtnGridQuery', {
    ...
    createQueryWin:function(){
    	...
    	if(me.queryWin == null){
		    ...
		    var getTreeText = function(bean){
		    	...
		    	var value = bean.fieldtype=='combobox'? boxValue.getRawValue():bean.fieldvalue;
    		...
    		var fieldcode = Ext.create('Ext.form.field.ComboBox',{emptyText:'字段名称',name:'fieldcode',datas:fields,width:150,allowBlank:false,
        		listeners:{
    		...
    		var operate = Ext.create('Ext.form.field.ComboBox',{emptyText:'比较符',name:'operate',viewname:'v_operate',allowBlank:false,
    			listeners:{
        			select: function(combo, record, eOpts){
        				textValue.setValue("");
    		...
    		var textValue = Ext.create('Ext.form.field.Text',{name:'fieldvalue',width:150,type:'string',emptyText:'比较符',allowBlank:false});
    		var numberValue = Ext.create('Ext.form.field.Number',{name:'fieldvalue',width:150,type:'number',hidden:true,disabled:true,allowBlank:false});
    		var boxValue = Ext.create('Ext.form.field.ComboBox',{name:'fieldvalue',width:150,type:'combobox',hidden:true,disabled:true,allowBlank:false});
    		var dateValue = Ext.create('Ext.form.field.Date',{name:'fieldvalue',width:150,type:'datetime',format : 'Y-m-d',hidden:true,disabled:true,allowBlank:false});
    		...
    		var buttons = [{text:'清除条件',scope:this,handler:function(){
    			...
    			var params = {autoParams:null,sys_grid_autoparams:null};
    			me.grid.load(params);
    			me.queryWin.hide();
    		}},{text:'查询',scope:this,handler:function(){
    			var datas = me.getData();
    			var params = {autoParams:true,sys_grid_autoparams:CU.toString(datas)};
    			me.grid.load(params);
    			me.queryWin.hide();
    		}}];
